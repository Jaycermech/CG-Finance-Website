...
async function addExpense(req, res) {
  try {
    ...
    const orgContent = await readJSON("utils/expenses.json");

    // Add the new expense to the existing array
    orgContent.push(newExpense);

    // Write the updated expenses back to the file
    await writeJSON(orgContent, "utils/expenses.json");

    // Respond with the updated expenses
    return res.status(201).json(orgContent);
  } catch (error) {
    return res.status(500).json({ message: error.message });
  }
}
